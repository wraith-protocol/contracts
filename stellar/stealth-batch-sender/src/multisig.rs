...
//! described in GOVERNANCE.md / MULTISIG.md and the shape of
//! stealth-sender/src/multisig.rs. This module only manages the governance
//! signer set + threshold; it is deliberately independent of `DataKey::Admin`
//! / `batch_send` logic so a bad rotation can never brick sends.

use soroban_sdk::{contracttype, Address, Env, Symbol, Vec};

use crate::{BatchSenderError, DataKey};
...
/// rotation function.
pub fn init(env: &Env, signers: Vec<Address>, threshold: u32) -> Result<(), BatchSenderError> {
    if env.storage().instance().has(&DataKey::MultisigSigners) {
        return Err(BatchSenderError::MultisigAlreadyInitialized);
    }
...
fn require_signer(env: &Env, caller: &Address) -> Result<(), BatchSenderError> {
    caller.require_auth();
    if !signers(env).contains(caller) {
        return Err(BatchSenderError::NotSigner);
    }
    Ok(())
}
...
fn validate_threshold(signers: &Vec<Address>, threshold: u32) -> Result<(), BatchSenderError> {
    if threshold == 0 || threshold > signers.len() {
        return Err(BatchSenderError::InvalidThreshold);
    }
    Ok(())
}
...
pub fn propose_rotate_signers(
    ...
    if !env.storage().instance().has(&DataKey::MultisigSigners) {
        return Err(BatchSenderError::MultisigNotInitialized);
    }
    require_signer(env, &caller)?;

    if env.storage().instance().has(&DataKey::PendingRotation) {
        return Err(BatchSenderError::RotationAlreadyPending);
    }
...
pub fn approve_rotate_signers(env: &Env, caller: Address) -> Result<(), BatchSenderError> {
    ...
    let mut proposal: RotationProposal = env
        ...
        .ok_or(BatchSenderError::NoPendingRotation)?;

    if proposal.approvals.contains(&caller) {
        return Err(BatchSenderError::AlreadyApprovedRotation);
    }
...
pub fn execute_rotate_signers(env: &Env, caller: Address) -> Result<(), BatchSenderError> {
    ...
    if (proposal.approvals.len()) < old_threshold {
        return Err(BatchSenderError::QuorumNotMet);
    }
    if env.ledger().timestamp() < proposal.executable_at {
        return Err(BatchSenderError::TimelockNotElapsed);
    }
...
pub fn cancel_rotate_signers(env: &Env, caller: Address) -> Result<(), BatchSenderError> {
    ...
    if !env.storage().instance().has(&DataKey::PendingRotation) {
        return Err(BatchSenderError::NoPendingRotation);
    }
    env.storage().instance().remove(&DataKey::PendingRotation);
    Ok(())
}
