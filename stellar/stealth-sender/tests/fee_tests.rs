...
use mocks::{token_auth_required::AuthRequiredToken, token_standard::StandardToken};
use soroban_sdk::{
...
fn test_fee_diverted_successfully() {
    ...
    h.sender_client().send(
        &h.sender, &token_id, &10_000, &1, &h.stealth, &h.epk, &h.meta,
    );
...
fn test_batch_fee_diverted_successfully() {
    ...
    h.sender_client().batch_send(
        &h.sender, &token_id, &1, &addresses, &epks, &metadatas, &amounts,
    );
...
fn test_fee_recipient_unauthorized_token_fails() {
    ...
    assert!(
        result.is_err(),
        "expected tx to revert since fee_recipient is unauthorized"
    );
