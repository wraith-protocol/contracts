...
describe('ERC5564Announcer', function () {
  ...
  it('should emit Announcement event with correct parameters', async function () {
    ...
    await expect(announcer.announce(schemeId, stealthAddress, ephemeralPubKey, metadata))
      .to.emit(announcer, 'Announcement')
      .withArgs(schemeId, stealthAddress, caller.address, ephemeralPubKey, metadata);
  });
  ...
  it('should allow multiple announcements from different callers', async function () {
    ...
    await expect(announcer.announce(schemeId, stealthAddress1, ephemeralPubKey, metadata))
      .to.emit(announcer, 'Announcement')
      .withArgs(schemeId, stealthAddress1, caller1.address, ephemeralPubKey, metadata);

    await expect(
      announcer.connect(caller2).announce(schemeId, stealthAddress2, ephemeralPubKey, metadata),
    )
      .to.emit(announcer, 'Announcement')
      .withArgs(schemeId, stealthAddress2, caller2.address, ephemeralPubKey, metadata);
  });
