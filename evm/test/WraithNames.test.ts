...
describe('WraithNames', function () {
  ...
  describe('register', function () {
    ...
    it('should emit NameRegistered event', async function () {
      ...
      await expect(names.register('alice', metaAddress, sig)).to.emit(names, 'NameRegistered');
    });
    ...
    it('should reject duplicate names', async function () {
      ...
      await expect(names.register('taken', m2.metaAddress, sig2)).to.be.revertedWithCustomError(
        names,
        'NameTaken',
      );
    });
    ...
    it('should reject names shorter than 3 characters', async function () {
      ...
      await expect(names.register('ab', metaAddress, sig)).to.be.revertedWithCustomError(
        names,
        'NameTooShort',
      );
    });
    ...
    it('should reject names with uppercase or special characters', async function () {
      ...
      await expect(names.register('Hello', metaAddress, sig)).to.be.revertedWithCustomError(
        names,
    ...
    it('should reject invalid signature (wrong signer)', async function () {
      ...
      await expect(names.register('hacker', metaAddress, sig)).to.be.revertedWithCustomError(
        names,
        'InvalidSignature',
      );
    });
  ...
  describe('registerOnBehalf', function () {
    ...
    it('should prevent replay with nonce', async function () {
      ...
      await expect(
        names.connect(relayer).registerOnBehalf('replay2', m2.metaAddress, sig2),
  ...
  describe('update', function () {
    ...
    it('should reject update from non-owner', async function () {
      ...
      await expect(names.update('owned', m2.metaAddress, wrongSig)).to.be.revertedWithCustomError(
        names,
        'NotOwner',
      );
    });
  ...
  describe('release', function () {
    it('should release a name and allow re-registration', async function () {
      ...
      const releaseSig = await (async () => {
        const digest = ethers.keccak256(ethers.solidityPacked(['string'], ['temporary']));
        return spendingWallet.signMessage(ethers.getBytes(digest));
      })();

      await expect(names.release('temporary', releaseSig)).to.emit(names, 'NameReleased');
